...
public class Node implements Cloneable {
    ...
    public void addRandomKids(NodeFactory nf, int maxDepth, Random rand) {
        ...
        if(left == null) {
            left = (rand.nextBoolean() ? nf.getOperator(rand) : nf.getTerminal(rand));
        }
        // Recursively add children for left if operator
        if(left.op instanceof Binop || left.op instanceof Unop) {
            left.addRandomKids(nf, maxDepth - 1, rand);
        }

        // Ensure right child exists if this is a binary operator
        if(op instanceof Binop) {
            if(right == null) {
                right = (rand.nextBoolean() ? nf.getOperator(rand) : nf.getTerminal(rand));
            }
            // Recursively add children for right if operator
            if(right.op instanceof Binop || right.op instanceof Unop) {
